...
function buildMeta(scene: SceneResponse): string {
  const parts: string[] = [];
  if (scene.difficulty_level) {
    parts.push(scene.difficulty_level.charAt(0).toUpperCase() + scene.difficulty_level.slice(1));
  }
...
export function SceneCard({ scene }: { scene: SceneResponse }) {
  ...
  return (
    <Card className="group relative flex flex-col gap-3 p-5 transition-shadow hover:shadow-md">
      ...
      <div className="pr-8">
        <h3 className="font-bold leading-tight tracking-tight">{scene.title}</h3>
